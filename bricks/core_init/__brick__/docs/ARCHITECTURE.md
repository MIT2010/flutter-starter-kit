...
Future<Result<Failure, User>> getUser(String id) async {
  try {
    ...
    _logger.e('getUser: unexpected', error: e, stackTrace: stackTrace);
...
- **`lib/core/services/`** — a cross-feature capability that has no screen
  (geolocation, a printer, a device-info provider). One `@lazySingleton`,
  returns `Result<Failure, S>` like a repository.
...
- **`lib/app/pages/`** and **`lib/app/widgets/`** — a static screen with
  no data layer (a consent gate, an "unsupported device" notice), and UI
  atoms shared by more than one feature.

More on both under "Where does this code go?".
